...
class StyleEmbeddingApiTestCase(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_market_style_embedding_is_repeatable_and_reusable_by_archetype_seed(self) -> None:
        statement_id = self._ingest_statement(
            "stmt_style_embedding",
        ...
        payload = first.json()
        self.assertIn("holding_period_distribution", payload)
        self.assertIn("momentum_preference_score", payload)
        self.assertIn("mean_reversion_score", payload)
        self.assertIn("concentration_score", payload)
        self.assertIn("reaction_latency_profile", payload)
    ...
    def test_market_style_embedding_marks_partial_when_upstream_features_are_thin(self) -> None:
        statement_id = self._ingest_statement(
            "stmt_style_embedding_small",
            "trade_date,symbol,side,quantity,price,fee,tax\n"
            "2026-03-14,600519.SH,buy,100,100,1,0\n"
            "2026-03-15,600519.SH,sell,100,101,1,0\n",
        )

        response = self.client.post(f"/api/v1/statements/{statement_id}/style-embedding")
