...
class CalibrationApiTestCase(unittest.TestCase):
    ...
    def test_distribution_calibration_builds_segment_and_pattern_stats(self) -> None:
        statement_id = self._ingest_statement(
            "stmt_distribution_calibration",
        ...
        response = self.client.post(f"/api/v1/statements/{statement_id}/distribution-calibration")
        self.assertEqual(response.status_code, 200)
        payload = response.json()
        self.assertGreaterEqual(len(payload["participant_segments"]), 1)
        self.assertGreaterEqual(len(payload["belief_patterns"]), 1)

    def test_weight_feedback_stays_partial_when_samples_are_thin(self) -> None:
        statement_id = self._ingest_statement(
            "stmt_weight_feedback",
            "trade_date,symbol,side,quantity,price,fee,tax\n"
            "2026-03-14,600519.SH,buy,100,100,1,0\n"
            "2026-03-15,600519.SH,sell,100,101,1,0\n",
        )

        response = self.client.post(f"/api/v1/statements/{statement_id}/weight-feedback")
        self.assertEqual(response.status_code, 200)
        payload = response.json()
        self.assertIn("participant_weights", payload)
        self.assertIn("scenario_weights", payload)
        self.assertFalse(payload["apply_allowed"])

    def _ingest_statement(self, statement_id: str, content: str) -> str:
        upload_response = self.client.post(
            "/api/v1/statements/upload",
            data={"owner_id": "calibration_user", "market": "CN_A", "statement_id": statement_id},
            files={"file": ("calibration.csv", content.encode("utf-8"), "text/csv")},
        )
