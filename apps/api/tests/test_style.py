...
class StyleCalibrationApiTestCase(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_activation_calibration_produces_minimum_event_type_mapping(self) -> None:
        statement_id = self._ingest_statement(
            "stmt_style_calibration",
        ...
        response = self.client.post(f"/api/v1/statements/{statement_id}/activation-calibration")
        self.assertEqual(response.status_code, 200)
        payload = response.json()
        self.assertIn("rules", payload)
        self.assertGreaterEqual(len(payload["rules"]), 1)
        self.assertEqual(payload["rules"][0]["event_type"], "supply_chain_price_shock")
        self.assertIn("participant_family", payload["rules"][0])

    def test_activation_calibration_can_fall_back_to_default_activation(self) -> None:
        statement_id = self._ingest_statement(
            "stmt_style_default",
            "trade_date,symbol,side,quantity,price,fee,tax\n"
        ...
        self.assertEqual(response.status_code, 200)
        payload = response.json()
        self.assertIn(payload["status"], {"partial", "default_activation", "ready"})
        self.assertGreaterEqual(len(payload["rules"]), 1)
