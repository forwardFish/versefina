...
class AgentValidationApiTestCase(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_agent_validation_supports_risk_noise_hold_and_grading_results(self) -> None:
        statement_id = self._ingest_statement(
            "stmt_agent_validation",
    ...
    def test_agent_validation_marks_provisional_when_samples_are_thin(self) -> None:
        statement_id = self._ingest_statement(
            "stmt_agent_validation_small",
            "trade_date,symbol,side,quantity,price,fee,tax\n"
            "2026-03-14,600519.SH,buy,100,100,1,0\n"
        ...
        self.assertEqual(payload["grading"], "provisional")

    def _ingest_statement(self, statement_id: str, content: str) -> str:
        upload_response = self.client.post(
            "/api/v1/statements/upload",
            data={"owner_id": "agent_user", "market": "CN_A", "statement_id": statement_id},
            files={"file": ("agent.csv", content.encode("utf-8"), "text/csv")},
        )
